...
$sql2 = "SELECT * from carrinho  where carrinho_veiculo = $veiculo and carrinho_status='1' and carrinho_cfc = $user[user_empresa] and carrinho_item = $item      ";
$exe2 = mysqli_query($conn, $sql2);
$totalitens =mysqli_num_rows($exe2);

if($qtd  == '0') { 

  $conn->query("DELETE from carrinho  where carrinho_item = $item and carrinho_status = '1' and carrinho_cfc = $user[user_empresa] and carrinho_veiculo = $veiculo  ");

}

if(($totalitens == '0') and ($qtd  <> '0')) {
@$conn->query($insert = "INSERT INTO carrinho (carrinho_item, carrinho_qtd, carrinho_usuario, carrinho_veiculo ,
...
'$veiculo','$user[user_empresa]','$cat','$sub','$oco','$valor','$desconto','$acrescimo')");

//print $insert;
 } else {
$conn->query("update carrinho set carrinho_qtd = $qtd , carrinho_desconto = $desconto , carrinho_acrescimo = $acrescimo where carrinho_veiculo = $veiculo and carrinho_status='1' and carrinho_cfc = $user[user_empresa] and carrinho_item = $item  ");
 }


$sql = "SELECT * from carrinho c inner join receitas_despesas rd ON c.carrinho_item = rd.receita_id where c.carrinho_veiculo = $veiculo and c.carrinho_status='1' and c.carrinho_cfc = $user[user_empresa] and c.carrinho_ocorrencia = '$oco' order by c.carrinho_id desc " ;
...
<script>
   ...
   $('#departamento').on('change', function() {
     ...
     $.ajax({
        url: 'parcelas_crediario_saida_veiculo.php?id='+id,
        cache: false,
        data: dados,
        type: "POST",  
...
<input type="hidden" name="veiculo" class="form-control" value="<?php echo $_POST[veiculo] ?>"> 
...
<thead >
                        <tr class="bg-warning text-white">
                           <th><strong class="text-white">Fornecedor</strong></th>
                           
                        </tr>
                        
                        <td>           <select name="fornecedor"   class="form-control">
<option value="">Informe </option>
<?php 
$sqli= "SELECT * FROM fornecedores where   fornecedor_status = 1 and fornecedor_cfc = '$user[user_empresa]' and fornecedor_lixeira = '1' ";
$exei = mysqli_query($conn, $sqli);
while($instrutor = mysqli_fetch_array($exei)) {
?>
<option value="<?php echo $instrutor[fornecedor_id] ?>" ><?php echo $instrutor[fornecedor_nome] ?></option>
<?php } ?>
</select>  </td>
