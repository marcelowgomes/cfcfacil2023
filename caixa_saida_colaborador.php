...
$sql2 = "SELECT * from carrinho  where carrinho_colaborador = $colaborador and carrinho_status='1' and carrinho_cfc = $user[user_empresa] and carrinho_item = $item      ";
$exe2 = mysqli_query($conn, $sql2);
$totalitens =mysqli_num_rows($exe2);

if($qtd  == '0') { 

  $conn->query("DELETE from carrinho  where carrinho_item = $item and carrinho_status = '1' and carrinho_cfc = $user[user_empresa] and carrinho_colaborador = $colaborador  ");

}

if(($totalitens == '0') and ($qtd  <> '0')) {
@$conn->query($insert = "INSERT INTO carrinho (carrinho_item, carrinho_qtd, carrinho_usuario, carrinho_colaborador ,
...
'$colaborador','$user[user_empresa]','$cat','$sub','$oco','$valor','$desconto','$acrescimo')");

//print $insert;
 } else {
$conn->query("update carrinho set carrinho_qtd = $qtd , carrinho_desconto = $desconto , carrinho_acrescimo = $acrescimo where carrinho_colaborador = $colaborador and carrinho_status='1' and carrinho_cfc = $user[user_empresa] and carrinho_item = $item  ");
 }


$sql = "SELECT * from carrinho c inner join receitas_despesas rd ON c.carrinho_item = rd.receita_id where c.carrinho_colaborador = $colaborador and c.carrinho_status='1' and c.carrinho_cfc = $user[user_empresa] and c.carrinho_ocorrencia = '$oco' order by c.carrinho_id desc " ;
...
<script>
   ...
   $('#departamento').on('change', function() {
     ...
     $.ajax({
        url: 'parcelas_crediario_saida_colaborador.php?id='+id,
        cache: false,
        data: dados,
        type: "POST",  
...
<input type="hidden" name="colaborador" class="form-control" value="<?php echo $_POST[colaborador] ?>"> 
...
var formData = new FormData(this);
  $.ajax({
    url: 'finalizarsaidacolaborador.php',
    cache: false,
    data: formData,
    type: "POST",  
