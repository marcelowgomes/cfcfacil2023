...
if($qtd  == '0') { 
  ...
  $conn->query("DELETE from carrinho  where carrinho_item = $item and carrinho_status = '1' and carrinho_cfc = $user[user_empresa] and carrinho_aluno = $aluno  ");
...
carrinho_cfc,carrinho_categoria, carrinho_subcategoria, carrinho_ocorrencia	,carrinho_valorun,
 carrinho_custo, carrinho_desconto, carrinho_acrescimo) VALUES ('$item','$qtd','$user[id_user]',
...
$sql = "SELECT * from carrinho c inner join receitas_despesas rd ON c.carrinho_item = rd.receita_id where c.carrinho_aluno = $aluno and c.carrinho_status='1' and c.carrinho_cfc = $user[user_empresa] and c.carrinho_ocorrencia = '$oco' order by c.carrinho_id desc " ;
...
<script>
   ...
   $('#departamento').on('change', function() {
     ...
     $.ajax({
        url: 'parcelas_crediario_saida.php?id='+id,
        cache: false,
        data: dados,
        type: "POST",  
